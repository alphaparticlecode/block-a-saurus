...
(function () {
  ...
  function Runner(outerContainerId, opt_config) {
    ...
    this.soundFx = {}; // Global web audio context for playing sounds.
  ...
  Runner.prototype = {
    ...
    adjustDimensions: function adjustDimensions() {
      ...
      this.dimensions.WIDTH = this.outerContainerEl.offsetWidth - padding * 2; // Redraw the elements back onto the canvas.
    ...
    playIntro: function playIntro() {
      if (!this.activated && !this.crashed) {
        ...
        var keyframes = '@-webkit-keyframes intro { ' + 'from { width:' + Trex.config.WIDTH + 'px }' + 'to { width: ' + this.dimensions.WIDTH + 'px }' + '}'; // create a style sheet to put the keyframe rule in 
        ...
        this.containerEl.style.width = this.dimensions.WIDTH + 'px'; // if (this.touchController) {
        //     this.outerContainerEl.appendChild(this.touchController);
        // }
    ...
    startGame: function startGame() {
      ...
      this.playCount++; // Handle tabbing off the page. Pause the current game.
  ...
  Runner.updateCanvasScaling = function (canvas, opt_width, opt_height) {
    var context = canvas.getContext('2d'); // Query the various pixel ratios
  ...
  GameOverPanel.prototype = {
    ...
    draw: function draw() {
      ...
      var centerX = this.canvasDimensions.WIDTH / 2; // Game over text.
      ...
      this.canvasCtx.drawImage(Runner.imageSprite, textSourceX, textSourceY, textSourceWidth, textSourceHeight, textTargetX, textTargetY, textTargetWidth, textTargetHeight); // Restart button.
  ...
  function checkForCollision(obstacle, tRex, opt_canvasCtx) {
    var obstacleBoxXPos = Runner.defaultDimensions.WIDTH + obstacle.xPos; // Adjustments are made to the bounding box as there is a 1 pixel white
    // border around the t-rex and obstacles.

    var tRexBox = new CollisionBox(tRex.xPos + 1, tRex.yPos + 1, tRex.config.WIDTH - 2, tRex.config.HEIGHT - 2);
    var obstacleBox = new CollisionBox(obstacle.xPos + 1, obstacle.yPos + 1, obstacle.typeConfig.width * obstacle.size - 2, obstacle.typeConfig.height - 2); // Debug outer box
    ...
    if (boxCompare(tRexBox, obstacleBox)) {
      ...
      var tRexCollisionBoxes = tRex.ducking ? Trex.collisionBoxes.DUCKING : Trex.collisionBoxes.RUNNING; // Detailed axis aligned box check.
  ...
  function Obstacle(canvasCtx, type, spriteImgPos, dimensions, gapCoefficient, speed, opt_xOffset) {
    ...
    this.collisionBoxes = [];
    this.gap = 0;
    this.speedOffset = 0; // For animated obstacles.
  ...
  function Trex(canvas, spritePos) {
    ...
    this.xPos = 0;
    this.yPos = 0; // Position when on the ground.
    ...
    this.timer = 0;
    this.msPerFrame = 1000 / FPS;
    this.config = Trex.config; // Current status.
  ...
  Trex.prototype = {
    ...
    draw: function draw(x, y) {
      ...
      sourceY += this.spritePos.y; // Ducking.
     ...
     * @param {number} speed
     */
    updateJump: function updateJump(deltaTime, speed) {
  ...
  NightMode.prototype = {
     ...
     * @param {number} delta
     */
    update: function update(activated, delta) {
      // Moon phase.
  ...
  HorizonLine.prototype = {
     ...
     */
    setSourceDimensions: function setSourceDimensions() {
  ...
  function Horizon(canvas, spritePos, dimensions, gapCoefficient) {
    ...
    this.spritePos = spritePos;
    this.nightMode = null; // Cloud

    this.clouds = [];
    this.cloudSpeed = this.config.BG_CLOUD_SPEED; // Horizon
  ...
  Horizon.prototype = {
    ...
    updateClouds: function updateClouds(deltaTime, speed) {
      ...
      if (numClouds) {
        ...
        var lastCloud = this.clouds[numClouds - 1]; // Check for adding a new cloud.
